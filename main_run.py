...
# also WeightedRandomSampler
from torch.utils.data import WeightedRandomSampler  # type: ignore[attr-defined]
...
# -----------------------------------------------------------------------------
# Distributed-training environment defaults & diagnostics (copied from Covid)
# -----------------------------------------------------------------------------
...
def get_args_parser():
    ...
    # Single-GPU convenience flag (parity with Covid script)
    parser.add_argument('--single_gpu', action='store_true',
                        help='Run on a single GPU without DDP.')
...
def train_dino(args):
    """Main training loop with single-GPU OR multi-GPU support."""
    ...
    if hasattr(args, 'single_gpu') and args.single_gpu:
        ...
        # Fake minimal distributed env so utils still work
        os.environ['RANK'] = '0'
        ...
        # ---- DISTRIBUTED (torchrun will set env vars) ----
        if not os.environ.get("CUDA_VISIBLE_DEVICES"):
    ...
    # ------- labeled subset for correction -------
    l_dataset = CXR_dataset.CXR_Dataset(
        args.data_path,
        csv_file='datafile.csv',
        transforms=transform,
        mode='train',
    ...
    if hasattr(args, 'single_gpu') and args.single_gpu:
        # Weighted sampler: give 3× weight to images with at least one positive symptom
        weights = [3.0 if l_dataset.total_images[img]["symptoms"].sum() > 0 else 1.0
                   for img in l_dataset.selected_images]
        w_sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)
        l_data_loader = DataLoader(
            l_dataset,
            sampler=w_sampler,
            batch_size=args.batch_size_per_gpu,
            num_workers=args.num_workers,
        ...
        l_sampler = DistributedSampler(l_dataset, shuffle=True)
        l_data_loader = DataLoader(
            l_dataset,
            sampler=l_sampler,
            batch_size=args.batch_size_per_gpu,
    ...
    if args.arch in vit_o.__dict__.keys():
        ...
        embed_dim = student.embed_dim  # type: ignore[attr-defined]
        inter_dim = 384
    ...
    teacher = utils.MultiCropWrapper(
        teacher,
        DINOHead(embed_dim, args.out_dim, args.use_bn_in_head), CLS_head(inter_dim, 256, 3), SymptomHead(inter_dim, 256, CXR_dataset.NUM_SYMPTOMS), args)

    # Load weights
    state_dict_raw = torch.load(args.pretrained_dir, map_location="cpu")
    # Remove symptom_head parameters that no longer match size
    state_dict = {k: v for k, v in state_dict_raw.items() if not k.startswith('symptom_head')}
    ...
    # ============ init schedulers ... ============
    # Learning-rate scaling consistent with Covid script
    if hasattr(args, 'single_gpu') and args.single_gpu:
        base_lr = args.lr
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, ce_loss, symptom_loss,
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, ce_loss, symptom_loss, data_loader, l_data_loader,
                    optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, ssl_epoch,
                    fp16_scaler, args):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _, useless) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output, teacher_cls, teacher_symptoms = teacher(images[:2])
            student_output, student_cls, student_symptoms = student(images)
            # print("**training**")
            # print("student")
            # print(student_cls)
            # print("student size")
            # print(student_cls.size())
            # print("student view ")
            # print(student_cls.view(-1))
            # print("student vew size")
            # print(student_cls.view(-1).size())
            # print("teacher")
            # print(teacher_cls)
            # print("teacher size")
            # print(teacher_cls.size())
            # print("teacher view ")
            # print(teacher_cls.view(-1))
            # print("teacher vew size")
            # print(teacher_cls.view(-1).size())
            loss_dino = dino_loss(student_output, teacher_output, epoch)

            temperature = 2.0  # This is a typical value, but it can be adjusted based on your model's needs
            teacher_probs = F.softmax(teacher_cls[:args.batch_size_per_gpu] / temperature, dim=1).detach()
            student_log_probs = F.log_softmax(student_cls[args.batch_size_per_gpu:] / temperature, dim=1)

            # KL divergence expects input=log_prob, target=probabilities
            loss_ce = F.kl_div(student_log_probs, teacher_probs, reduction='batchmean')
            loss_symptom= symptom_loss(student_symptoms[args.batch_size_per_gpu:].view(-1, CXR_dataset.NUM_SYMPTOMS), torch.sigmoid(teacher_symptoms[:args.batch_size_per_gpu].view(-1, CXR_dataset.NUM_SYMPTOMS)))
            loss_ce = (loss_ce + loss_symptom) /2.0
            if args.alpha:
        ...
        with torch.no_grad():
            ...
            if hasattr(args, 'single_gpu') and args.single_gpu:
                param_iter = zip(student.parameters(), teacher_without_ddp.parameters())
            else:
                param_iter = zip(student.module.parameters(), teacher_without_ddp.parameters())

            for param_q, param_k in param_iter:
        ...
        if it % args.correct == 0:
            ...
            for _, (images, labels, symptom_labels) in enumerate(metric_logger.log_every(l_data_loader, 10, header)):
                # move images to gpu
                images = [im.cuda(non_blocking=True) for im in images]
                labels = labels.long().cuda()
                symptom_labels = torch.tensor(symptom_labels).float().cuda()
                ...
                with torch.cuda.amp.autocast(fp16_scaler is not None):
                    ...
                    l_loss_ce = ce_loss(student_cls[args.batch_size_per_gpu:], labels.view(-1))
                    symptoms_loss = symptom_loss(student_symptoms[args.batch_size_per_gpu:], symptom_labels)

                    l_loss_ce = (l_loss_ce + symptoms_loss) / 2.0

                    symptom_labels = symptom_labels.repeat(2, 1)

                    symptoms_loss = symptom_loss(student_symptoms, symptom_labels)
                    loss = (l_loss_ce + symptoms_loss)/ 2.0
...
# ------------------ Focal BCE loss for imbalance ------------------


class FocalBCE(nn.Module):
    """Binary cross-entropy with logits + focal modulation."""
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.global_transfo2 = transforms.Compose([
            transforms.RandomResizedCrop(256, scale=global_crops_scale, interpolation=transforms.InterpolationMode.BICUBIC),
        ...
        self.local_transfo = transforms.Compose([
            transforms.RandomResizedCrop(128, scale=local_crops_scale, interpolation=transforms.InterpolationMode.BICUBIC),
            flip_and_color_jitter,
            transforms.RandomRotation(degrees=(-15, 15)),
...
# -----------------------------------------------------------------------------
# Entry-point: auto-launch torchrun when needed (parity with Covid script)
# -----------------------------------------------------------------------------
if __name__ == '__main__':
    parser = argparse.ArgumentParser('DINO', parents=[get_args_parser()])
    args = parser.parse_args()

    # Already in a torchrun context?
